...
- **Astra Cybersecurity Local Audit Remediation (Lots 1 to 4)**:
  ...
  - **Lot 2 (SQL Column Bounds, Path Confinement & Transaction Atomicity)**: Clamped folder name length to 64 chars matching `Folder.name VARCHAR(64)`; validated image name length (<= 64) and description length (<= 256) matching `Image` schema; enforced path confinement checks via `is_relative_to` in folder creation, image upload, image replacement, item deletion, and file-serving endpoints (`files.py` and `mobile_api.py`); added automatic rollback compensatory file deletion on `db.session.commit()` failures in image and folder operations.
  - **Lot 3 (Single-Use Password Reset Token & Complete Cascade Deletion)**: Bound password reset tokens to SHA-256 fingerprint of `user.password_hash`, automatically invalidating reset tokens once consumed or upon password change; updated `delete_account` to fully delete user `ProfileTree`, `Profile`, and `PrintOption` records, preventing foreign key constraint violations and orphaned data.
  - **Lot 4 (Resource Budgets & Client-Side Isolation)**: Validated profile bounds (name <= 64, remote_avatar_url <= 256) and deduplicated tree associations in `save_profile`; clamped print preview `gridMultiplier` to safe range (1..50) in `ListPdfExporter.js`; scoped print preset `localStorage` keys per user ID (`picto_print_presets_${userId}`); escaped Treant node image URLs before template interpolation.
  - **Validation & Test Suite**: Added dedicated security tests in `tests/test_security.py` for folder name bounds, profile bounds, and tree association deduplication; verified token replay protection and cascading account deletion in `tests/test_auth.py`; full test suite passes with 100% success rate (Ruff, ESLint, 54 Node JS tests, 66 Pytest tests, 11 Playwright E2E tests).
